...
// In-memory cache of last raw email-accounts API response for debugging
let _lastEARawDebug = null;
...
const server = http.createServer((req, res) => {
  ...
  // ── Return last raw EA debug snapshot ───────────────────────────────────
  if (pathname === "/api/debug-ea-raw" && req.method === "GET") {
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify(_lastEARawDebug || { error: "No snapshot yet — load Email Accounts page first" }, null, 2));
    return;
  }

  // ── Debug: trace the full "In Campaign" detection flow ─────────────────
  if (pathname === "/api/debug-in-campaign" && req.method === "GET") {
    const allAccs = readAccounts();
    const acc = allAccs.find(a => a.api_key);
    if (!acc) {
      res.writeHead(400, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ error: "No account configured" }));
      return;
    }
    const apiKey = acc.api_key;
    function dbGet(p) {
      return new Promise(resolve => {
        const opts = {
        ...
        const pr = https.request(opts, pres => {
          ...
          pres.on("end", () => {
            ...
            catch { resolve({ status: pres.statusCode, body: buf.slice(0, 500) }); }
          });
        });
        pr.on("error", e => resolve({ status: 0, error: e.message }));
        pr.setTimeout(12000, () => { pr.destroy(); resolve({ status: 0, error: "timeout" }); });
        pr.end();
      });
    }
    (async () => {
      // Step 1: Fetch first page of accounts and campaigns
      const [accountsRes, campRes] = await Promise.all([
        dbGet("/api/v2/accounts?limit=5"),
        dbGet("/api/v2/campaigns?limit=5"),
      ]);
      const accounts = Array.isArray(accountsRes.body?.items) ? accountsRes.body.items
                     : Array.isArray(accountsRes.body)        ? accountsRes.body : [];
      const campaigns = Array.isArray(campRes.body?.items) ? campRes.body.items
                      : Array.isArray(campRes.body)        ? campRes.body : [];

      // Step 2: Show what fields are on each account object (look for tags)
      const accountFieldSample = accounts.length > 0 ? {
        all_keys: Object.keys(accounts[0]),
        tag_like_fields: Object.fromEntries(
          Object.entries(accounts[0]).filter(([k]) => /tag|label|group|list|campaign/i.test(k))
        ),
        first_account_email: accounts[0].email || accounts[0].address || "?",
      } : { note: "No accounts found" };

      // Step 3: Get first tag UUID from campaigns
      const firstTagId = campaigns.find(c => Array.isArray(c.email_tag_list) && c.email_tag_list.length > 0)
                                   ?.email_tag_list[0] ?? null;

      // Step 4: Try ALL plausible tag→accounts parameter variants
      let tagVariants = {};
      if (firstTagId) {
        const variants = [
          `tag_id=${firstTagId}`,
          `tag_ids=${firstTagId}`,
          `tags=${firstTagId}`,
          `email_tag_id=${firstTagId}`,
          `email_tag_ids=${firstTagId}`,
          `label_id=${firstTagId}`,
          `label=${firstTagId}`,
          `group_id=${firstTagId}`,
          `filter[tag_id]=${firstTagId}`,
        ];
        const variantResults = await Promise.all(
          variants.map(qs => dbGet(`/api/v2/accounts?${qs}&limit=5`))
        );
        variants.forEach((qs, i) => {
          const r = variantResults[i];
          const items = Array.isArray(r.body?.items) ? r.body.items
                       : Array.isArray(r.body)       ? r.body : [];
          tagVariants[qs] = { status: r.status, count: items.length, emails: items.map(a => a.email||"?") };
        });
      }

      // Step 5: Reverse lookup — for first 3 accounts, check what campaigns they're in
      // This is the confirmed-working approach from previous debug
      const sampleEmails = accounts.slice(0, 3).map(a => a.email || a.address).filter(Boolean);
      const reverseLookups = {};
      await Promise.all(sampleEmails.map(email =>
        dbGet(`/api/v2/campaigns?email_account=${encodeURIComponent(email)}&limit=5`)
          .then(r => {
            const items = Array.isArray(r.body?.items) ? r.body.items : Array.isArray(r.body) ? r.body : [];
            reverseLookups[email] = {
              status: r.status,
              campaigns_count: items.length,
              campaign_names: items.map(c => c.name).slice(0, 5),
              body_preview: items.length === 0 ? (r.body?.error || r.body) : undefined,
            };
          }).catch(e => { reverseLookups[email] = { error: String(e) }; })
      ));

      // Step 6: Try email_account_tag list endpoint directly
      const emailTagEndpoints = await Promise.all([
        dbGet("/api/v2/email-account-tags?limit=10"),
        dbGet("/api/v2/account-tags?limit=10"),
        dbGet("/api/v2/tags?limit=10"),
      ]);

      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({
        step2_account_field_sample: accountFieldSample,
        step3_first_tag_id_from_campaign: firstTagId,
        step4_tag_param_variants: tagVariants,
        step5_reverse_lookup_campaigns_per_account: reverseLookups,
        step6_tag_list_endpoints: {
          "email-account-tags": { status: emailTagEndpoints[0].status, body: emailTagEndpoints[0].body },
          "account-tags":       { status: emailTagEndpoints[1].status, body: emailTagEndpoints[1].body },
          "tags":               { status: emailTagEndpoints[2].status, body: emailTagEndpoints[2].body },
        },
        guidance: "Look at step4 for which tag→accounts param works (status 200 + count > 0), OR step5 for reverse lookup",
      }, null, 2));
    })();
    return;
  }

  // ── Debug: probe campaign→account link endpoints ────────────────────────
  if (pathname === "/api/debug-campaigns" && req.method === "GET") {
    const allAccs = readAccounts();
    const acc = allAccs.find(a => a.api_key);
    if (!acc) {
      res.writeHead(400, { "Content-Type": "application/json" });
    ...
    function iGet(p) {
      return new Promise(resolve => {
        ...
        const pr = https.request(opts, pres => {
          ...
          pres.on("end", () => {
            try { resolve({ status: pres.statusCode, body: JSON.parse(buf) }); }
            catch { resolve({ status: pres.statusCode, body: buf.slice(0, 300) }); }
          });
        });
        pr.on("error", e => resolve({ status: 0, error: e.message }));
        pr.setTimeout(10000, () => { pr.destroy(); resolve({ status: 0, error: "timeout" }); });
        pr.end();
      });
    }
    // From debug-campaigns run: first campaign has email_tag_list with these two UUIDs
    const TAG_ID_1 = "d91998f2-187d-43af-a582-e082ce75adf6";
    const TAG_ID_2 = "133f0a40-529f-4e7f-86af-f0cb62f2665c";
    // A known account email visible in Instantly UI screenshot
    const SAMPLE_EMAIL = encodeURIComponent("ideus@reactionspaceventure.com");

    // Probe all plausible endpoints in parallel
    Promise.all([
      // Can we get accounts filtered by a tag UUID from email_tag_list?
      iGet(`/api/v2/accounts?tag_id=${TAG_ID_1}&limit=100`),
      iGet(`/api/v2/accounts?email_tag_id=${TAG_ID_1}&limit=100`),
      iGet(`/api/v2/accounts?label_id=${TAG_ID_1}&limit=100`),
      // Can we get campaigns for a specific email account?
      iGet(`/api/v2/campaigns?email_account=${SAMPLE_EMAIL}&limit=10`),
      iGet(`/api/v2/campaigns?account_email=${SAMPLE_EMAIL}&limit=10`),
      iGet(`/api/v2/campaigns?sending_account=${SAMPLE_EMAIL}&limit=10`),
      // Single account detail (might include tags or campaign refs)
      iGet(`/api/v2/accounts/${SAMPLE_EMAIL}`),
      // Label/group listing endpoints
      iGet(`/api/v2/labels?limit=100`),
      iGet(`/api/v2/email-labels?limit=100`),
      iGet(`/api/v2/account-labels?limit=100`),
      iGet(`/api/v2/sending-groups?limit=100`),
    ]).then(results => {
      const keys = [
        "accounts?tag_id={uuid}",
        "accounts?email_tag_id={uuid}",
        "accounts?label_id={uuid}",
        "campaigns?email_account={email}",
        "campaigns?account_email={email}",
        "campaigns?sending_account={email}",
        "accounts/{email} (single)",
        "labels",
        "email-labels",
        "account-labels",
        "sending-groups",
      ];
      const out = { note: "Round 2 — probing tag/account/campaign cross-endpoints" };
      keys.forEach((k, i) => {
        out[k] = { status: results[i].status, body: results[i].body };
      });
      res.writeHead(200, { "Content-Type": "application/json" });
  ...
  if (pathname === "/api/email-accounts" && req.method === "GET") {
    ...
    (async () => {
      try {
        ...
        // ── Save raw snapshot for debugging via /api/debug-ea-raw ────────
        const rawFirstAccounts = emailAccounts.slice(0, 5);
        const rawFirstCampaign = allCampaigns.length > 0 ? allCampaigns[0] : null;
        _lastEARawDebug = {
          timestamp: new Date().toISOString(),
          accounts_sample: rawFirstAccounts,
          accounts_sample_keys: rawFirstAccounts.length > 0 ? Object.keys(rawFirstAccounts[0]) : [],
          alt_accounts_status: altAccountsRes.status,
          alt_accounts_body_keys: altAccountsRes.body && typeof altAccountsRes.body === 'object' ? Object.keys(altAccountsRes.body) : [],
          campaigns_total: allCampaigns.length,
          active_campaigns_total: allCampaigns.filter(c => Number(c.status) === 1).length,
          active_campaigns_sample: allCampaigns.filter(c => Number(c.status) === 1).slice(0, 5).map(c => ({ name: c.name, status: c.status, email_tag_list: c.email_tag_list })),
          tag_ids_found: allTagIds.size,
          campaign_email_set_size: campaignEmailSet.size,
          campaign_email_set_sample: [...campaignEmailSet].slice(0, 10),
          first_campaign_email_tag_list: rawFirstCampaign?.email_tag_list ?? null,
          first_campaign_keys: rawFirstCampaign ? Object.keys(rawFirstCampaign) : [],
          custom_tags_total: Object.keys(instantlyTagById).length,
          custom_tags_sample: Object.values(instantlyTagById).slice(0, 10),
          custom_tag_ids_sample: Object.keys(instantlyTagById).slice(0, 5),
          email_tag_ids_sample: [...allTagIds].slice(0, 5),
          ids_overlap: [...allTagIds].filter(id => instantlyTagById[id]).length,
          emailToCustomTagIds_size: Object.keys(emailToCustomTagIds).length,
          emailToCustomTagIds_sample: Object.entries(emailToCustomTagIds).slice(0, 5).map(([k,v]) => ({ email: k, tagIds: v })),
          note: "campaignEmailSet built via email_tag_list → /api/v2/accounts?tag_id=",
        };
        flog("[email-accounts] raw snapshot saved — fetch /api/debug-ea-raw to inspect");
